...
The purpose of this project is to allow the conversion of your saved games from `MHXX (3DS)` to `MHGU (Switch)` and vice versa, using three scripts I've developed through reverse engineering to assist you with this challenging task. While there's already an app called [MHXXSwitchSaveEditor](https://github.com/Dawnshifter/MHXXSwitchSaveEditor) that performs this task, it's not particularly effective for converting saved games; its strength lies more in editing them.
...
### Option B
An alternative option for adding DLCs consists in following the steps detailed in sections [2](#2-mhgu-to-mhxx---switch-to-3ds) or [3](#3-mhxx-to-mhgu---3ds-to-switch) and then load the generated files in the corresponding console in order to download the DLCs.
...
6. To make sure that the generated file is not damaged, I recommend testing it on your Switch or emulator. If everything goes well, you should find your 3DS version chats in the chat menu. If you choose `option 1` in the script, you may see some strange characters, which is normal since some characters of the 3DS version are not supported in the Switch version and vice versa. However, they should now be visible and not appear empty. If you edited them in English, they should display correctly in both versions. On the other hand, if you choose `option 2`, they will be overwritten with the English version of MHGU in all your saved slots.
7. If everything goes well, you don't need to continue reading; go directly to section [2](#2-mhgu-to-mhxx---switch-to-3ds) or simply use your new file with the fixed chats on your Switch or emulator.
...
1. Place your Switch MHGU `system` file in the root folder of the `MHGU_TO_MHXX - (Switch To 3DS)` folder.
...
1. Place your 3DS MHXX `system` file in the root folder of the `MHXX_TO_MHGU - (3DS To Switch)` folder.
